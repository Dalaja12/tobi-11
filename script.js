...
// 🖼️ Buscar imágenes sin API ni fetch (funciona en local y Kodular)
async function showImageResults(query) {
    try {
        const encoded = encodeURIComponent(query);
        const googleUrl = `https://www.google.com/search?tbm=isch&q=${encoded}`;
        const container = document.createElement("div");
        container.className = "image-results";
        container.innerHTML = `
            <p>🖼️ No puedo mostrar imágenes dentro del chat (por seguridad),<br>
            pero puedo abrir una búsqueda de imágenes:</p>
            <a href="${googleUrl}" target="_blank" class="image-link">🔗 Ver imágenes de ${query}</a>
        `;
        document.getElementById("chatContainer").appendChild(container);
    } catch (error) {
        console.error(error);
        addMessage("Error al mostrar imágenes 😢", "bot");
    }
}

// 🎬 Buscar videos con miniaturas (sin API ni fetch)
async function showVideoResults(query) {
    try {
        const encoded = encodeURIComponent(query);
        const youtubeUrl = `https://www.youtube.com/results?search_query=${encoded}`;

        // Crear HTML para mostrar miniaturas falsas (solo visual)
        const container = document.createElement("div");
        container.className = "video-results";

        // Mostrar miniaturas simuladas (solo 3 ejemplos)
        for (let i = 0; i < 3; i++) {
            const randomId = Math.random().toString(36).substring(2, 8); // ID temporal
            const thumbUrl = `https://img.youtube.com/vi/${randomId}/hqdefault.jpg`;
            const card = document.createElement("div");
            card.className = "video-card";
            card.innerHTML = `
                <img src="${thumbUrl}" alt="${query}">
                <p class="video-title">Videos de ${query}</p>
            `;
            card.onclick = () => window.open(youtubeUrl, "_blank");
            container.appendChild(card);
        }

        document.getElementById("chatContainer").appendChild(container);
        addMessage("Haz clic en un video para ver en YouTube 🎬", "bot");
    } catch (error) {
        console.error(error);
        addMessage("Error al mostrar videos 😢", "bot");
    }
}
